...
The [plugin API](/docs/api-reference/plugins) enables developers to share extensions to Frontend.
...
Frontend provides official plugins that can be installed for common use-cases:

- [`ai`](https://editoreditor.com/docs/ai/overview): Use AI to generate pages using your own components.
- [`emotion-cache`](https://github.com/editoreditor/editor/tree/main/packages/plugin-emotion-cache): Inject emotion cache into the Frontend iframe.
- [`heading-analyzer`](https://github.com/editoreditor/editor/tree/main/packages/plugin-heading-analyzer): Analyze the heading outline of your page and be warned when you're not respecting WCAG 2 accessibility standards.

Please see the [awesome-editor repo](https://github.com/editoreditor/awesome-editor) for a full list of community plugins.
...
If you're familiar with Frontend, you can likely already build a Frontend plugin. See the [Plugin API reference](/docs/api-reference/plugins) for a full breakdown of available APIs.
...
const myPlugin = {
  ...
  icon: <Coffee />, // Icon shown in the rail (use lucide to match Frontend)
  render: () => <div>My plugin UI</div>, // Component rendered in plugin panel
};
```

You can leverage the [internal Frontend API](internal-editor-api) to integrate Frontend behavior with your plugin:
...
Plugins support [UI Overrides](/docs/extending-editor/ui-overrides), enabling you to override discrete section of the Frontend interface.
...
import { registerOverlayPortal } from "@frontend/core";
